...
export function useSepaCharts(
  ...
  useEffect(() => {
    ...
    const lineH52w = makeTogglableLine(candle, {
      price: chart.high52w,
    ...
    const lineExt = chart.extendedLine
      ? makeTogglableLine(candle, {
          price: chart.extendedLine,
          color: theme.down,
          lineWidth: 1,
          lineStyle: 3,
          title: 'MA50 +25% extended',
        })
      : null;
    ...
    const observers = [
      ...
      observeSize(rsEl, rsChart),
      observeSize(vrEl, vrChart),
    ];
        ...
        items: [
          {
            ...
            toggle: (v) => ma50.applyOptions({ visible: v }),
          },
          {
            key: 'ma150',
            label: 'MA150',
            color: seriesPalette[4],
            toggle: (v) => ma150.applyOptions({ visible: v }),
          },
          {
            key: 'ma200',
            label: 'MA200',
            color: seriesPalette[1],
            toggle: (v) => ma200.applyOptions({ visible: v }),
          },
        ],
      },
      {
            ...
            ? [
                {
                  ...
                  label: 'MA50 +25%',
                  color: theme.down,
                  toggle: (v: boolean) => lineExt.set(v),
                },
              ]
            : []),
        ],
