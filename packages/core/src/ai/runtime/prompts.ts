...
// K-line arrives as a table, not JSON. Every payload repeats its own header, so a
// prompt that forgets to mention the format still leaves the model a way in; this
// paragraph adds what a header cannot carry — that extended-hours volume is not
// comparable with regular-session volume.
export const KLINE_TABLE_FORMAT =
  'K-line comes as a plain-text table, not JSON: two "#" lines give the symbol/period/timezone/scope and the column names, then rows follow. "## <date> <session>" starts a block and applies to every row beneath it until the next one; session is pre (pre-market and overnight), reg (regular session) or post (after-hours). Row times are already converted to the market timezone — do not shift them. An empty field is null (indicators are empty until they have enough history). Extended-hours bars are thin: never compare pre/post volume against reg volume as though they were the same scale.';
...
export const ANALYST_ADAPTER_PROMPT = [
  ...
  '- Always include eps_pe_plan in submit_prediction. Follow the activated eps-pe-scenario-stock-analysis skill (Darren EPS × PE). Source consensus forward EPS / PE / PEG / Wall Street target via web_search when they are not in data_snapshot. Three scenarios (bear/base/bull) are required; if a number is assumed, label it in sources.',
...
export const COMMENTATOR_PROMPT = [
  ...
  'At the start of a session you receive a JSON snapshot with live quotes, capital flow, an archived intraday-prediction summary, recent comments, and the trigger reason, followed by a five-minute K-line table carrying MACD. For later triggers on the same day you receive only an incremental update (current quote, new bars, capital-flow tail); earlier snapshots and your own prior comments remain above in this conversation — use them directly. The full snapshot arrives under the JSON key "pack" and an incremental one under "update".',
  KLINE_TABLE_FORMAT,
...
export const EXPLAINER_PROMPT = [
  ...
  "You receive a JSON snapshot with the live quote, capital flow, today's archived intraday-prediction plan (entry / stop / targets / zones, when one exists), recent comments, the day levels (previous day, pre-market range, opening range), and relative volume, followed by a five-minute K-line table carrying MACD.",
  KLINE_TABLE_FORMAT,
