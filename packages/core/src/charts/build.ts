...
function requireSymbol(body: Body, type: string): string {
  ...
  if (typeof symbol !== 'string' || !symbol) {
    throw new ClientError(
      `${type}: \`symbol\` is required when \`data\` is not provided`,
      'e.g. {"type": "' + type + '", "symbol": "NVDA.US"}',
    );
  }
  return symbol;
}
  ...
  switch (type) {
    case 'sepa': {
      ...
      const [kline, spyKline, news, name] = await Promise.all([
        (async () =>
          (body.kline as RawBar[] | undefined) ??
          (await provider.getKline(symbol, 'day', count)))(),
        (async () =>
          (body.spy_kline as RawBar[] | undefined) ??
          (body.skip_spy === true
            ? []
            : await getProvider('US').getKline('SPY.US', 'day', count)))(),
    ...
    case 'cohort': {
      ...
      if (!Array.isArray(rows) || !rows.length) {
        throw new ClientError(
          'cohort: `data` must be a non-empty array',
          'e.g. [{"symbol": "MU", "value": -17087}]',
        );
      }
...
export function rebuild(
  ...
  input: Record<string, unknown>,
  title?: string,
): Omit<BuildResult, 'input'> & { input: Record<string, unknown> } {
  switch (type) {
    case 'sepa': {
      const { built, meta } = buildSepa(input as unknown as SepaInput);
      const symbol = built.sidebar.symbol;
      const sessionDate =
        (built.sidebar.asOf && marketSessionDate(symbol, built.sidebar.asOf)) || localToday();
      return {
        type,
    ...
    case 'cohort': {
      ...
      const sessionDate =
        type === 'flow' && typeof lastTime === 'string' ? ymd(lastTime) : localToday();
      return {
        type,
...
export function migrateLegacyDoc(doc: ChartDoc): ChartDoc {
  ...
  const needsRebuild =
    kind === 'echarts' || (kind === 'intraday' && hasLegacyOffSession(doc.built));
...
function hasLegacyOffSession(built: unknown): boolean {
  const tfs = (built as { timeframes?: Record<string, { offSession?: unknown[] }> } | undefined)
    ?.timeframes;
  if (!tfs) return false;
