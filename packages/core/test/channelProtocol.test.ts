...
import { runAssistantChatTurn } from '../src/ai/assistant/assistantChat.js';
import type { AnalystDeps } from '../src/ai/personas/analyst/types.js';
import { runAnalyst } from '../src/ai/personas/analyst/run.js';
import { createAssistantSession } from '../src/ai/assistant/assistantChatStore.js';
...
describe('assistant-chat channel', () => {
  it('pushes init, forwards live events, and stops after unsubscribe', async () => {
    ...
    conn.emitMessage(
      JSON.stringify({ op: 'sub', key: 'assistant1', kind: 'assistant-chat', id: session.id }),
    );
    ...
    const evenMoreMessage = {
      role: 'assistant',
      content: [{ type: 'text', text: '分析中更多结论' }],
      timestamp: Date.now(),
    };
    listener?.({
      type: 'message_update',
      ...
      assistantMessageEvent: {
        ...
        contentIndex: 0,
        delta: '结论',
        partial: evenMoreMessage as never,
      },
    });
      ...
      await waitFor(() =>
        conn.sent.some((raw) => {
          ...
          return (
            ...
            msg.payload.status.phase === 'researching'
          );
        }),
      );
      ...
      await waitFor(() =>
        conn.sent.some((raw) => {
          ...
          return (
            msg.payload.type === 'update' &&
            msg.payload.symbol === symbol &&
            msg.payload.status.running === false
          );
        }),
      );
      await run.done;
...
describe('parseWsMessage annotations kind', () => {
  it('parses a valid annotations subscription', () => {
    expect(
      parseWsMessage({ op: 'sub', key: 'k1', kind: 'annotations', symbol: 'NVDA.US' }),
    ).toEqual({
      op: 'sub',
      key: 'k1',
      kind: 'annotations',
