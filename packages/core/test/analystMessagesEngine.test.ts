...
describe('AnalystMessagesEngine', () => {
  it('builds an ephemeral model view without mutating or accumulating on the raw transcript', async () => {
    ...
    const raw: AgentMessage[] = [{ role: 'user', content: '请重估 MU.US。', timestamp: 1 }];
  ...
  it('recomputes high-churn run state at the tail without persisting the old value', async () => {
    ...
    expect(textOf(after.messages.at(-1)!)).toContain(
      '<data_as_of>2026-07-15T15:30:00.000Z</data_as_of>',
    );
    expect(textOf(after.messages.at(-1)!)).not.toContain(
      '<journal_written>false</journal_written>',
    );
