        ...
        timeframes: {
          m5: {
            bars: [
              ...
              bar('2026-07-13T17:55:00Z', 1005, 990, 1002),
            ],
            summary: null,
          },
          m15: { bars: [], summary: null },
...
describe('rejectAnswer — the code-level half of TD-VERIFY-01', () => {
  ...
  it('accepts the correction', () => {
    expect(
      rejectAnswer({ claim_status: 'contradicted', verification_id: 'v1' }, minted),
    ).toBeNull();
  });
  ...
  it('accepts partial and insufficient too — the gate blocks sycophancy, not disagreement', () => {
    ...
    expect(
      rejectAnswer({ claim_status: 'insufficient', verification_id: 'v1' }, minted),
    ).toBeNull();
  });
  ...
  it('forces insufficient when the data is incomplete — no taking sides on missing data', () => {
    ...
    expect(rejectAnswer({ claim_status: 'contradicted', verification_id: 'v2' }, m)).toContain(
      'insufficient',
    );
